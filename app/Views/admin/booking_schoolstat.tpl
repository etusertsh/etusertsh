                        ...
                        <table class="table table-striped table-hover">
                            <thead>
                                <tr>
                                    <th>學制</th>
                                    <th>學校名稱</th>
                                    <th class="text-primary h5">核定數</th>
                                    <th class="text-success h5">填報數</th>
                                    <th class="text-danger h5">剩餘</th>
                                    <th></th>
                                </tr>
                            </thead>
                            <tbody>
                            ...
                            <tr{{if $limitdata.$key.remain<=0}} style="background: {{if $limitdata.$key.limitnum==0}}#EEE{{else}}#FFFFCC{{/if}};"{{/if}}>
                                    ...
                                    <td class="text-primary h5">{{$limitdata.$key.limitnum}}</td>
                            <td class="text-success h5"{{if $limitdata.$key.used>0}} style="background: #EDED00;"{{/if}}>{{$limitdata.$key.used}}</td>
                                    <td class="text-danger h5">{{$limitdata.$key.remain}}</td>
